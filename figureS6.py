...
badstas = ['PALK','PTCN','BFO','CMLA','OBN','SFJD']
for sta in badstas:
    for tr in st.select(station=sta):
        st.remove(tr)

inv += client.get_stations(network='II', station='BFO', starttime=eve, 
                endtime = eve + hours*60*60, level="response", location='10', channel='LDO')


st += client.get_waveforms(network='II', station='BFO', location='10', channel='LDO', 
            starttime=eve, endtime=eve + hours*60*60)
...
fig, ax = plt.subplots(len(st), 1, figsize=(16,22))
ax = ax.flatten()
...
for idx, tr in enumerate(st):
    ...
    ax[idx].set_yticklabels([tr.stats.station], fontsize=12)
...
plt.savefig('FigureS6.PDF', format='PDF', dpi=400)
plt.close('all') 
